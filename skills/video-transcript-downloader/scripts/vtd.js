...
function resolveFirecrawl() {
  return resolveBin("firecrawl");
}
...
function extractTranscriptSection(markdown) {
  if (!markdown) return "";
  const patterns = [
    /^## Transcript\s*\n([\s\S]*?)(?=\n##\s+|$)/m,
    /^### Transcript\s*\n([\s\S]*?)(?=\n###\s+|\n##\s+|$)/m,
...
function hasTimestampMarkers(text) {
  if (!text) return false;
  return /(^|\n)\s*(?:\[\d{1,2}:\d{2}(?::\d{2})?\]|\d{1,2}:\d{2}(?::\d{2})?)\s+\S/m.test(text);
}
...
async function firecrawlYouTubeTranscript(url) {
  const firecrawl = resolveFirecrawl();
  if (!firecrawl) return null;

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vtd-firecrawl-"));
  const tmpPath = path.join(tmpDir, "result.json");
  const actions = [
    { type: "wait", milliseconds: 7000 },
    {
      type: "executeJavascript",
      script:
        '(()=>{ const b=[...document.querySelectorAll("button")].find(el=>((el.getAttribute("aria-label")||el.innerText||"").includes("Transcript"))); if (b) { b.click(); return "clicked"; } return "missing"; })()',
    },
    { type: "wait", milliseconds: 5000 },
  ];

  try {
    const r = await run(firecrawl, [
      "scrape",
      url,
      "--actions",
    ...
    if (r.code !== 0 || !fs.existsSync(tmpPath)) return null;

    const payload = JSON.parse(fs.readFileSync(tmpPath, "utf8"));
    const markdown = typeof payload?.markdown === "string" ? payload.markdown : "";
    const metadata = payload?.metadata && typeof payload.metadata === "object" ? payload.metadata : {};
    const transcript = extractTranscriptSection(markdown);
    const title = metadata.title || url;
    const id = metadata.identifier || metadata.videoId || metadata.id || extractYouTubeId(url) || title;
...
async function cmdTranscript({ url, lang, timestamps, keepBrackets, extra }) {
  ...
  if (isYouTubeUrl(url)) {
    const firecrawlResult = await firecrawlYouTubeTranscript(url);
    if (firecrawlResult?.transcript) {
      if (timestamps && hasTimestampMarkers(firecrawlResult.transcript)) {
        process.stdout.write(firecrawlResult.transcript.trim() + "\n");
        return;
      }
      const paragraph = toParagraph(firecrawlResult.transcript.split(/\r?\n/), { keepBrackets });
      if (paragraph) {
    ...
    if (id) {
      try {
        // Legacy fallback: direct transcript fetch when Firecrawl doesn't surface one.
